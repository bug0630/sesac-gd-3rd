...
// btn1: 익명 함수, 함수 표현식 사용해 변쉥 저장하지 않고 작성
...
// hover는 마우스가 해당 요소 위에 올라가 있응ㄹ 때만 적용
btn1.addEventListener("mouseout", function () {
  this.style.backgroundColor = "black";
});

// btn2 : 화살표 함수 사용해 container 에 요소 추가
btn2.addEventListener("click", () => {
  const div = document.createElement("div");
  div.style.backgroundColor = "yellowgreen";
  div.style.marginBottom = "10px";
  div.innerText = "btn 2를 클릭해 추가된 div";
...
function changecolor() {
  ...
  //   만약 div 요소가 없다면 alert창 띄우는 예외 처리
  if (divs.length !== 0) {
    for (let div of divs) {
      div.style.color = "red";
    }
